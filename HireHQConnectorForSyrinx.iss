...
begin
  ...
  begin
    ...
    SaveStringToFile(ExpandConstant('{app}\download-hire-contract-files.bat'),
      ...
      'gcloud auth activate-service-account --key-file="{commonappdata}\HireHQConnector\service-account.json"' + #13#10 +
      'gcloud storage rsync --recursive "gs://hirehq-app-production-hire-contracts/' + TenantId + '" "' + SyrinxDirPage.Values[0] + '\Docs\Hire Contracts\" --exclude="pending./.*" --dry-run', False);
  end;
end;
...
[UninstallRun]
Filename: "schtasks"; Parameters: "/Delete /TN ""Hire HQ Sync Driver Photos"" /F"; Flags: runhidden; RunOnceId: syncDriverTask
Filename: "schtasks"; Parameters: "/Delete /TN ""Hire HQ Sync Fleet Documents"" /F"; Flags: runhidden; RunOnceId: syncFleetTask
Filename: "schtasks"; Parameters: "/Delete /TN ""Hire HQ Download Hire Contract Files"" /F"; Flags: runhidden; RunOnceId: syncContractTask
Filename: "schtasks"; Parameters: "/Delete /TN ""Hire HQ Connector Tray"" /F"; Flags: runhidden; RunOnceId: trayAppTask
